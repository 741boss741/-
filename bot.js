...
client.on('message',async message => {
  ...
  if(message.content.startsWith(prefix + "giveaway")) {
    ...
    message.channel.send(`:eight_pointed_black_star:| **منشن الروم الي انتا عايز فيها الجيف اواي**`).then(msgg => {
      message.channel.awaitMessages(filter, {
        ...
        if(!room) return message.channel.send(':heavy_multiplication_x:| **مقدرتش الاقي الروم المطلوبه**');
        room = collected.first().content;
        collected.first().delete();
        msgg.edit(':eight_pointed_black_star:| **اكتب مدة الجيف اواي**').then(msg => {
          message.channel.awaitMessages(filter, {
            max: 1,
            time: 20000,
            errors: ['time']
            ...
            collected.first().delete();
            msgg.edit(':eight_pointed_black_star:| **واخيرا اكتب الجيف اواي علي ايه ؟**').then(msg => {
              message.channel.awaitMessages(filter, {
                max: 1,
                time: 20000,
                errors: ['time']
                ...
                try {
                  ...
                  message.guild.channels.find('name', room).send(giveEmbed).then(m => {
                     ...
                     setTimeout(() => {
                       ...
                       .setAuthor(message.author.username, message.author.avatarURL)
                       .setTitle(title)
                       .addField('الجيف اواي خلص !',`الفائز هو : ${gFilter}`)
